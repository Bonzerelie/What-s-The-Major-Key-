...
(() => {
  ...
  const LS_KEY_NAME = "ftk_player_name";
  ...
  const FINAL_AVG_TEXT = (percent) => {
      if (percent < 25) return "You're down but you're not out! Give it another go and see if you can improve ☝️";
      if (percent < 50) return "That's not a bad way to begin, but I reckon you've got a higher score in you!";
      if (percent < 75) return "That's not bad at all, though the higher scores are calling your name 😉";
      if (percent < 100) return "That's pretty great! A score to be proud of, but can you go one further? 💪🧐";
      return "Perfect score! Nice one! Time to move on to the next game in the series!";
  };
  ...
  async function openStartModal() {
    ...
    showModal({
      bodyHtml:
      `<img src="images/title.png" alt="Title Image" style="width: 60%; height: auto; margin-bottom: 15px;"><br>` +  
      `Begin by pressing <strong>Play Track</strong>. You'll hear a piano recording and need to figure out which <strong>Major key</strong> it belongs to. If the track sounds as though it's in a Minor key, find the <strong>relative Major</strong>.<br>` +
        `Use the keyboard to audition notes. Find the <strong>root note</strong> of the Major scale and click <strong>submit</strong>.<br><br>` +
...
I have, for the last decade or so, almost always viewed all music in Major or Minor keys through the lens of a the Major scale and have personally found this incredibly useful as a very fast way to learn and navigate through, particularly pop rock, music. Music will not always be presented to you in this way, and you don’t have to tackle music from this angle yourself - results may vary - but being aware of this as an option could be useful.
`,
      withKeyboard: false,
      actions: backToStart
        ? [{
  ...
  function showSummary() {
    ...
    const html = `
      <div class="scoreItem"><span class="scoreK">Questions Asked</span><span class="scoreV">${asked}</span></div>
      <div class="scoreItem"><span class="scoreK">Answers Correct</span><span class="scoreV">${correct}</span></div>
      <div class="scoreItem"><span class="scoreK">Correct in a row</span><span class="scoreV">${score.streak}</span></div>
      <div class="scoreItem"><span class="scoreK">Longest Streak</span><span class="scoreV">${Math.max(score.longest, score.streak)}</span></div>
      <div class="scoreItem"><span class="scoreK">Percentage Correct</span><span class="scoreV">${percent}%</span></div>
      <div class="scoreItem scoreItemFull" style="margin-top: 8px;"><span class="scoreK">Keys Included</span><span class="scoreV" style="font-size:14px;">${keysIncluded}</span></div>
      <div style="margin-top: 16px; font-weight: 600; opacity: 0.9;">${FINAL_AVG_TEXT(percent)}</div>
    `;
    if(summaryScoreOut) summaryScoreOut.innerHTML = html;
    
    if (modalPlayerNameInput) {
      const savedName = localStorage.getItem(LS_KEY_NAME) || "";
      modalPlayerNameInput.value = savedName.slice(0, 32);
    }
  ...
  summaryModal?.addEventListener("click", async (e) => { 
      if (e.target === summaryModal) hideSummaryHandler(); 
  });
  ...
  async function downloadScoreCardPng(playerName) {
    ...
    if (name) {
      try { localStorage.setItem(LS_KEY_NAME, name.slice(0, 32)); } catch {}
    }
    ...
    const rows = [
      ...
      ["Answers Correct", String(correct)],
      ["Correct in a row", String(score.streak)],
      ["Longest Streak", String(Math.max(score.longest, score.streak))],
      ["Percentage Correct", `${percent}%`],
    ];
    ...
    const keysH = 80;
    const H = baseContentH + (rows.length * (rowH + 14)) + keysH + 80; 
    ...
    const titleImg = await new Promise(res => { 
      const img = new Image(); 
      img.onload = ()=>res(img); 
    ...
    // Keys Included specific row 
    ctx.fillStyle = "#ffffff";
    drawRoundRect(ctx, rowX, yCursor, rowW, keysH, 14);
    ctx.fill();
    ctx.strokeStyle = "rgba(0,0,0,0.16)";
    ctx.stroke();
    ...
    ctx.textAlign = "center";
    ctx.fillText("Keys Included", rowX + rowW / 2, yCursor + 28);
    
    ctx.fillStyle = "#111";
    ctx.font = "800 15px Arial, Helvetica, sans-serif";
    ctx.fillText(formatAllowedRoots(allowedRoots), rowX + rowW / 2, yCursor + 54);
    ...
    ctx.fillText("What's The Major Key?! - www.eartraininglab.com", W / 2, cardY + cardH - 24);
  ...
  // --------------------
  // Title image swap + embed resizing
  // --------------------
  ...
  function bind() {
    ...
    resetBtn.addEventListener("click", () => {
      stopVoices(["track", "note", "feedback"], 0.08); // Ensure audio stops immediately 
      if (score.asked > 0) {
        showSummary();
      } else {
        resetToInitial();
    ...
    // Handle the main UI download button
    ...
    // Handle the summary modal UI download button
    ...
    document.addEventListener("keydown", async (e) => {
      ...
      if (summaryModal && !summaryModal.classList.contains("hidden")) {
          if (e.code === "Escape") await hideSummaryHandler();
          return;
      }
